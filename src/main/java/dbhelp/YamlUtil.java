 ...
 * @param
 * @version 1.0
 * @Description yaml 数据及文件转换和处理
 * @Author pengweitao 2022/5/8 下午3:12
 * @exception
 * @return
 */
public class YamlUtil {
     ...
     *
     * @param ymlFileName 工程根目录下resource目录,文件名称格式:/resource/abc.yml
     * @return List<Node> 每个Nyml 文件中每行对应解析的数据
     */
    public List<YmlNode> castProperties(String ymlFileName) {
        if (ymlFileName == null || ymlFileName.isEmpty() || !ymlFileName.endsWith(".yml")) {
            throw new RuntimeException("请输入yml文件名称！！");
        }
        //URL url = this.getClass().getResource(ymlFileName);
        File ymlFile = new File(ymlFileName);
        if (!ymlFile.exists()) {
            throw new RuntimeException("/resource目录下不存在 " + ymlFileName + "文件！！");
        }
     ...
     */
    public Map<String, String> getPropertiesMap(String ymlFileName) {
     ...
     * @return
     */
    public Properties toProperties(String ymlFileName){
        Map<String,String> ymldata = this.getPropertiesMap(ymlFileName);
        Properties pro = new Properties();
        Set keySet = ymldata.keySet();
        keySet.forEach((k)->{pro.setProperty((String) k,ymldata.get(k));});
return pro;
    }
    ...
    private   String printNodeList(List<YmlNode> nodeList) {
    ...
    private   List<YmlNode> getNodeList(String yml) {
    ...
    private   boolean getNodeLast(String key, Set<String> keySet) {
        if (key.isEmpty()) {
            return true;
        }
        key = key + ".";
    ...
    private   YmlNode getNode(String line) {
